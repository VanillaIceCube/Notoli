...
- `ROBOCOP_APP_ID` repository variable and `ROBOCOP_PRIVATE_KEY` repository secret authenticate the RoboCop GitHub App. Install it with `Contents: read`, `Pull requests: write`, `Checks: read`, `Actions: read`, `Security events: read`, and `Issues: write`. The alert workflows request only `Security events: read` and `Issues: write` in their short-lived installation tokens; the other permissions remain available only to RoboCop's PR-review workflow.
...
- RoboCop must have `Issues: write` for issue creation, edits, labels, assignment, comments, and closure, plus `Security events: read` for CodeQL and Dependabot alert reads. After adding `Issues: write` to the App registration, approve the requested permission update on the existing installation. Each workflow explicitly narrows the one-hour installation token to those two permissions and to the Notoli repository.
