...
5) For local Docker runs that should use the current checkout rather than published GHCR images, rebuild first:
   ...
   - `docker build --build-arg REACT_APP_API_BASE_URL=https://localhost/apps/notoli -t ghcr.io/vanillaicecube/notoli-frontend:latest ./frontend`
6) The included reverse proxy serves the frontend at:
