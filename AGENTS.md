...
- Public URLs (subdomain-root):
  ...
  - Backend:
    - `https://notoli.judeandrewalaba.com/api`
      ...
      - Notifications use `GET /api/notifications/`, `PATCH /api/notifications/<id>/`, and `PATCH /api/notifications/mark-all-read/`.
...
- Frontend auth behavior:
  ...
  - Any backend `401` from non-auth endpoints clears tokens and redirects to `/login` and shows an error snackbar on the login page.
