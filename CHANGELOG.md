...
### Changed
- Automated frontend and backend lint-fix commits now use a short-lived Lint Eastwood GitHub App token and the app bot's dynamically resolved noreply identity, while fork and Dependabot pull requests use a no-secret, strict-check-only lint path.
